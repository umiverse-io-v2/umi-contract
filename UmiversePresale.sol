...
import "./UmiverseNFT.sol";
import "./lib/SafeMath.sol";
import {ERC2771Context} from "@gelatonetwork/relay-context/contracts/vendor/ERC2771Context.sol";

contract UmiversePresale is ERC2771Context {
    ...
    function setUmiverseNFT(UMIVERSE _umiverseNFT) public onlyOwner {
        umiverseNFT = _umiverseNFT;
    }
    ...
    function presale(
        ...
        if (price > 0) {
            ...
            processTrade(sendToken, sender, receiver, price);
        }

        orders[nftid] = Order(sender, receiver, price, _priceType, timestamp);
        nextOrderId += 1;

        // uint256[] memory dynamicArray = new uint256[](nftid);
        // umiverseNFT.batchMint(dynamicArray, sender);
        emit Presale(
            sender,
            receiver,
            nftid,
    ...
    function getMessageHash(
        ...
        require(isTokenSupported(priceType), "token not support");
        return
            keccak256(
                abi.encodePacked(
                    sender,
